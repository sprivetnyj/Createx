...
export function counter(counterArray) {
	counterArray.forEach((counterItem, index) => {
		// data-counter="value" - для блока цифр
		...
		function counterAnim(timestapm) {
			...
			if (counterItem.closest('[data-circle]')) {
				const newValue = value.slice(0, value === '100' ? 3 : 2);
				const colors = ['248,152,40', '245,47,110', '90,135,252', '3,206,164'];
				counterItem.closest('[data-circle]').style.background =
					`conic-gradient(
						rgb(${colors[index]}) ${easeInOutCubic(progress, 0, newValue, duration) * 3.6}deg,
						rgba(${colors[index]}, 0.3) ${easeInOutCubic(progress, 0, newValue, duration) * 3.6}deg
					)`;
			}
		}
...
export function headerScroll() {
	...
	if (header) {
		...
		// При скролле
		document.addEventListener("scroll", scrollStyles);
		// С первого экрана
		scrollStyles();
